...
function QuineMcCluskeyDataCtrl() {
  ...
  this.compute = function() {
          ...
          for (var j = i + 1; j < lastIg.group.length; j++) {
            ...
            if (imp1.bitMask === imp2.bitMask) {
              ...
              if (found) {
                ...
                for (var m in imp1.imp) impl.imp[m] = parseInt(m);
                for (var n in imp2.imp) impl.imp[n] = parseInt(n);
            ...
            for (var thisVal in primTerm.implicant.imp) {
              ...
              for (var v = 0; v < this.noOfVars; v++) {
                if ((needed & one) === one) {
                  if ((thisVal & one) === one) {
                    minTerm =
                      "<i>x</i><sub><small>" + v + "</small></sub>" + minTerm;
                  } else {
                    minTerm =
                      "<i>x&#772;</i><sub><small>" +
                      v +
              ...
              var colorStr =
                ...
                primTerm.color[2] +
                ")";
              primTerm.coloredTermString =
                "<span style='color:" + colorStr + "'>" + minTerm + "</span>";
              break;
            }
    ...
    this.primTermTables.length = 0;
    var primTableLoop = 0;
    var primTablFound = this.primTerms.length > 0;
      ...
      if (primTableLoop === 0) {
        ...
        for (var k = 0; k < prevTable.remainingPrimTerms.length; k++) {
          if (!prevTable.remainingPrimTerms[k].used) {
            ...
            for (
              var l = 0;
              l < prevTable.remainingPrimTerms.length && !superseded;
              l++
            ) {
            ...
            if (!superseded) {
              primTermTable.remainingPrimTerms.push(
                prevTable.remainingPrimTerms[k]
              );
            } else {
              prevTable.supersededPrimTerms.push(
                prevTable.remainingPrimTerms[k]
              );
            }
          }
        ...
        for (var i = 0; i < solutions.length; i++) {
          ...
          if (count <= bestCount) {
            ...
            if (count === bestCount) {
              ...
              for (var j in solutions[i]) {
                for (var v in primTermTable.remainingPrimTerms[j].implicant
                  .imp) {
                  bestVarCountNew++;
                }
              }
    ...
    if (solutionFound) {
      ...
      this.minimalTerm =
        'Error: The cyclic covering problem is too large (increase the "maxProblemSize" parameter)';
      this.coloredMinimalTerm =
        'Error: The cyclic covering problem is too large (increase the "maxProblemSize" parameter)';
    }
  };
...
function QuineMcCluskey(parentDivId, columns, language) {
  ...
  this.init = function() {
    ...
    if (!myDiv) {
      ...
      if (!parent) {
        if (divId !== "fakeDivId") {
          console.log(
            "QuineMcCluskey error: can not find an element with the given name: " +
              divId
          );
        }
        myDiv = -1;
  ...
  this.drawImplicantGroup = function(g, parent, primFlag, t, drawPetrickVars) {
    ...
    for (var i = 0; i < iMax; i++) {
      ...
      if (!primFlag) {
        ...
        cellLast.setAttribute("class", "qmcTdNoBorder");
        cellLast.innerHTML =
          primTermTable.remainingPrimTerms[i].coloredTermString;
        if (drawPetrickVars) {
          var pVars =
            "&nbsp;&equiv;&nbsp;<i>p</i><sub><small>" + i + "</small></sub>";
  ...
  this.update = function() {
    ...
    for (var j = 0; j < this.cols; j++) {
      ...
      if (j < this.cols - 1) {
        myCell.innerHTML =
          "<i>x</i><sub><small>" + (this.cols - 2 - j) + "</small></sub>";
        myCell.setAttribute("class", "qmcHeaderX qmcBit");
      } else {
    ...
    for (var i = 0; i < this.data.implicantGroups.length; i++) {
      ...
      this.drawImplicantGroup(
        this.data.implicantGroups[i],
        myImplicantDiv,
        false,
        0,
        false
    ...
    for (var i = 0; i < this.data.primTermTables.length; i++) {
      ...
      if (i === 0) {
        ...
        resultDiv.innerHTML =
          "<p> " + labels["primChartReduced"] + " " + (i - 1) + "):</p>";
      }
      ...
      if (
        this.data.petrickTermPrims.length > 0 &&
        i === this.data.primTermTables.length - 1
      ) {
      ...
      if (jj > 0) {
        essPTermsDiv.innerHTML =
          "<p>" +
          labels["extractedPrims"] +
          ": <span class='qmcMathFont'>" +
          essPTermsStr +
          "</span></p>";
    ...
    if (this.data.petrickTermPrims.length > 0) {
      ...
      petrickInnerDiv.innerHTML =
        "<span class='qmcMathFont'>" + this.data.petrickSolver.log + "</span>";
      ...
      if (jj > 0) {
        petrickEssTermsDiv.innerHTML =
          "<p>" +
          labels["extractedMPrims"] +
          " (" +
          labels["petricksM"] +
          "): <span class='qmcMathFont'>" +
          petrickEssTermsStr +
    ...
    termDiv.innerHTML =
      "<p><strong>" +
      labels["minExp"] +
      ":</strong> </p> <p ><span class='qmcMathFont'><i>y</i>&nbsp;=&nbsp;" +
      this.data.coloredMinimalTerm;
    +"</span></p>";
...
module.exports = { qm: QuineMcCluskeyDataCtrl };
